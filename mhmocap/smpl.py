...
ModelOutput = namedtuple('ModelOutput', ['vertices', 'joints','joints_h36m17', 'joints_smpl24'])
...
def create(model_path, model_type='smpl', **kwargs):
    ''' Method for creating a model from a path and a model type
        ...
        ----------
        model_path: str
            Either the path to the model you wish to load or a folder that contains the model.
        model_type: str
        **kwargs: dict
            Keyword arguments
...
class SMPL(nn.Module):
    ...
    def __init__(self, model_path,
            ...
            J_reg_alphapose_path=None,
            J_reg_mupots_path=None,
            data_struct=None,
            betas=None,
            global_orient=None,
            body_pose=None,
        ...
        if J_reg_alphapose_path is not None:
            J_regressor_alphapose = np.load(J_reg_alphapose_path).T
            J_regressor_alphapose = to_tensor(to_np(J_regressor_alphapose), dtype=dtype)
            self.register_buffer('J_regressor_alphapose', J_regressor_alphapose)
        else:
            self.register_buffer('J_regressor_alphapose', None)

        if J_reg_mupots_path is not None:
            J_regressor_mupots = np.load(J_reg_mupots_path).T
            J_regressor_mupots = to_tensor(to_np(J_regressor_mupots), dtype=dtype)
            self.register_buffer('J_regressor_mupots', J_regressor_mupots)
        else:
            self.register_buffer('J_regressor_mupots', None)
    ...
    def forward(self, batch_size=512, **kwargs):
        ...
        if person_num > batch_size and batch_size > 0:
            ...
            for inds in range(int(np.ceil(person_num/float(batch_size)))):
                ...
                batch_data = {k : v[inds * batch_size : (inds + 1) * batch_size] for k, v in kwargs.items()}
    ...
    def single_forward(self, betas=None, poses=None,
        ...
        if self.J_regressor_alphapose is not None:
            joints_alphapose = vertices2joints(self.J_regressor_alphapose, vertices)
            outputs.update({'joints_alphapose': joints_alphapose})

        if self.J_regressor_mupots is not None:
            joints_mupots = vertices2joints(self.J_regressor_mupots, vertices)
            outputs.update({'joints_mupots': joints_mupots})
...
if __name__ == '__main__':
    ...
    smpl_model_parameters_path = '/CT/depth-3dhp/work/git/hd3d-dataset/model_data/parameters'
    ...
    SMPLPY = SMPL(
        ...
        J_reg_h36m17_path=smpl_J_reg_h37m_path).to(device)
    ...
    if visualization:
        ...
        mesh = vedo.Mesh([verts, faces]).c('w').alpha(0.8)
        # joints_smpl24 = vedo.Points(results['joints_smpl24'][0].cpu().numpy().tolist()).c('red')
        # joints_h36m17 = vedo.Points(results['joints_h36m17'][0].cpu().numpy().tolist()).c('blue')
        j3d = vedo.Points(results['j3d'][0].cpu().numpy().tolist()).c('green')
        # vedo.show(mesh, joints_smpl24, viewup="z", axes=1)
        vedo.show(mesh, j3d, viewup="z", axes=1)
        # vedo.show(mesh, joints_h36m17, viewup="z", axes=1)
